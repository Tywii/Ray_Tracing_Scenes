...
Intersection Sphere::getIntersection(Ray ray){
	...
	//vec3 pc = centre;
	//float r = radius;
	vec3 D = ray.origin - centre;
	float delta = pow(dot(d, D), 2.0f) - pow(length(D), 2.0f) + pow(radius, 2.0f);
	float t;
	if (delta < 0) {
		...
		i.point = ray.origin + t * d;
		i.normal = normalize(i.point - centre);
	}

	else {
		t = -dot(d, D) - sqrt(delta);
		//float t2 = -dot(d, D) - sqrt(delta);
		i.point = ray.origin + t * d;
		i.normal = normalize(i.point - centre);
